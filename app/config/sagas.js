...
export const getCoinList = () => fetch('http://coincap.io/coins/');

export const getLatestRate = currency => fetch(`http://coincap.io/page/${currency}`);
...
const fetchLatestConversionRates = function* ({ currency }) {
  ...
  try {
    ...
    if (usedCurrency === undefined) {
      usedCurrency = yield select(state => state.currencies.quoteCurrency);
    }
